...
@implementation RCTYdPlatelineView {
    NSString *_stockCode;
}
...
//        self.changeData = @[
//            @{@"code":@"sh300001", @"name":@"证券", @"index":@27, @"rise_rate":@-1.3},
//            @{@"code":@"sh300002", @"name":@"酒店及餐饮", @"index":@37, @"rise_rate":@-2.3},
//            @{@"code":@"sh300002", @"name":@"电子制造", @"index":@47, @"rise_rate":@-1.7},
//            @{@"code":@"sh300002", @"name":@"仪器仪表", @"index":@147, @"rise_rate":@1.7},
//            @{@"code":@"sh300002", @"name":@"生物制品", @"index":@149, @"rise_rate":@1.7},
//            @{@"code":@"sh300003", @"name":@"石油矿业开采", @"index":@158, @"rise_rate":@-1.3},
//            @{@"code":@"sh300003", @"name":@"汽车", @"index":@168, @"rise_rate":@-1.3}
//        ];
    }
    return self;
}
...
//    mainTextRect = CGRectMake(0, top, self.bounds.size.width, height);
    ...
    top += height;
    height = (self.bounds.size.height - top - 24);// - 20 ) * 3.0 / 4 ;
...
//异动数据
- (void)setPlateData:(NSString *)plateData {
    _plateData = plateData;
    
    NSError *error;
    NSData *data =[_plateData dataUsingEncoding:NSUTF8StringEncoding];
    NSDictionary *plateDataDic = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableLeaves error:&error];
    self.changeData = plateDataDic[@"plateData"];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (!self.isTingPan) {
        ...
        //板块异动
        [self drawChange:context region:mainChartRect axisY:mainYAxis dravArray:pricearray points:self.changeData formula:mainFormula];
    }
...
- (void)drawChange:(CGContextRef)context region:(CGRect)region axisY:(YdYAxis)axisY dravArray:(NSArray*)points points:changeData formula:(shared_ptr<Formula>)formula {
    ...
    CGFloat x_rectangle_last = 0;
    CGFloat y_rectangle_last = 0;
    CGFloat textSize_width_last = 0;
    
    for (int i=0; i<self.changeData.count; i++) {
        //数据
//        NSString *blockCode = self.changeData[i][@"code"]; //板块代码
        NSString *blockName = self.changeData[i][@"name"];
        int index = [self.changeData[i][@"index"] intValue];
        id item = points[index];
        CGPoint currentPoint = CGPointFromString(item);
        CGFloat x = index*[self getWidthForMinute];    //索引*线间隔
        CGFloat y = currentPoint.y;
        CGFloat rise_rate = [self.changeData[i][@"rise_rate"] floatValue];
        UIColor * offsetColor = (rise_rate>=0) ? RGB(245, 25, 63) :  RGB(92, 174, 22);
        CGFloat offsetLength = (y<=region.size.height/2) ? 20 : -20;
        CGFloat offsetSep = (y<=region.size.height/2) ? 25 : -25;
        
        //计算文字和框位置
        UIFont  *font = [UIFont boldSystemFontOfSize:12.0];
        NSDictionary *attr = @{ NSFontAttributeName:font,
                                NSForegroundColorAttributeName:offsetColor};
        NSDictionary *attribute = @{NSFontAttributeName: font};
        CGFloat textSize_width = [blockName boundingRectWithSize:CGSizeMake(1000,30)
                                                         options:
                              NSStringDrawingTruncatesLastVisibleLine |
                              NSStringDrawingUsesLineFragmentOrigin |
                              NSStringDrawingUsesFontLeading
        ...
        CGFloat x_rectangle = x;
        CGFloat y_rectangle = y+offsetLength+offsetSep;
        if (x_rectangle > region.size.width/2) {
            x_rectangle = x_rectangle-textSize_width;
        }
        if ((x_rectangle < x_rectangle_last+textSize_width_last) && (y_rectangle_last+offsetSep>0 && y_rectangle_last+offsetSep*2<region.size.height)) {
            y_rectangle = y_rectangle_last+offsetSep;
        }
        x_rectangle_last = x_rectangle;
        y_rectangle_last = y_rectangle;
        textSize_width_last = textSize_width;
        
        //点位
        CGContextSetFillColorWithColor(context, offsetColor.CGColor);
        CGContextAddArc(context, x, y, 3, 0, 2*M_PI, 0); //添加一个圆
        CGContextDrawPath(context, kCGPathFill);//绘制填充
        
        //线
        const CGFloat *colors = CGColorGetComponents(offsetColor.CGColor);
        CGContextSetRGBStrokeColor(context, colors[0], colors[1], colors[2] ,1.0);
        CGContextSetLineWidth(context,1.0);
        CGMutablePathRef path = CGPathCreateMutable();
        CGPathMoveToPoint(path, nullptr, x, y);
        CGPathAddLineToPoint(path, nullptr,x, y_rectangle);
        ...
        //文字边框
        CGContextAddRect(context, CGRectMake(x_rectangle, y_rectangle, textSize_width, 20));
        CGContextStrokePath(context);
        
        //文字
        [blockName drawAtPoint:CGPointMake(x_rectangle+5, y_rectangle+3) withAttributes:attr];
    }
    CGContextRestoreGState(context);
}
